type PasswordViewProps = {
  password: string;
  onCopyPasswordClickHandler: (password: string) => void;
};

export const PasswordView = ({
  onCopyPasswordClickHandler,
  password,
}: PasswordViewProps) => {
  return (
    <div className={"px-8 py-5 flex bg-darkGray items-center min-w-full gap-4"}>
      <span className={"select-none text-almostWhite text-headerL text-nowrap"}>
        {password}
      </span>
      <button
        className={"ml-auto flex justify-center items-center"}
        onClick={() => onCopyPasswordClickHandler(password)}
      >
        <svg
          width="21"
          height="24"
